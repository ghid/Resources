...
rem pushd %gitlogpath%
rem for /F %%l in (conemu_git_%ConEmuServerPID%_1.log) do call :calc "%%l"
rem popd
...
rem if "%gitbranch_add% %gitbranch_chg% %gitbranch_del%" == "0 0 0" (
  set "gitbranch= %ESC%[36;40m(%gitbranch%)"
rem ) else (
  rem set "gitbranch= [%gitbranch% +%gitbranch_add% ~%gitbranch_chg% -%gitbranch_del%]"
rem )
rem echo "%gitbranch%"
